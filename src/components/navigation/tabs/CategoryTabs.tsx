...
import { blueBorder, blueText } from "../../../variables/styles/colors";
...
const CategoryTabs: FC<Props> = ({activeCategory, setActiveCategory}) => {
        ...
        <div className="border-b border-gray-200 dark:border-gray-700">
            <ul className="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400">
                {categories.length > 0 && categories.map(item => (
                    <li onClick={() => setActiveCategory(item)} className="me-2" key={item.id}>
                        <button type="button" className={`cursor-pointer p-4 border-b-2 ${flex_center} 
                        ${activeCategory.id == item.id ? blueText : "text-gray-500 dark:text-gray-400 hover:text-gray-600"} 
                        ${activeCategory.id == item.id ? blueBorder : "border-transparent hover:border-gray-300"}`} aria-current="page">
                            {item.name}
                        </button>
                    </li>
                ))}
                {/* <li className="me-2">
                    <a href="#" className="inline-flex items-center justify-center p-4 border-b-2 rounded-t-lg hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 group">
