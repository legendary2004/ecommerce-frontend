...
import { bluebgColor, blueText, primaryBgColors, secondaryTextColor } from "../variables/styles/colors";
import { flex_center } from "../variables/styles/display/flex";
import { maxScreenXl } from "../variables/styles/size";
import { sectionHeader, headerText } from "../variables/styles/text";
import BackHome from "../components/buttons/BackHome";
import { authPage } from "../interface/componentsInt";
import { useNavigate } from "react-router-dom";
import { FormEvent } from "react";

const AuthPage: React.FC<authPage> = ({children, label, page, navigation, formSubmit}) => {
    const navigate = useNavigate()
    return (
        <Theme>
            <div className={primaryBgColors}>
            <div className={`min-h-screen ${flex_center} ${maxScreenXl}`}>
                    <div className="grid md:grid-cols-2 items-center gap-10 max-w-6xl max-md:max-w-md w-full">
                        <div>
                            <BackHome />
                            <h2 className={`${sectionHeader} text-start`}>
                                Seamless Login for Exclusive Access
                            </h2>
                            <p className={`${secondaryTextColor}`}>Immerse yourself in a hassle-free login journey with our intuitively designed login form. Effortlessly access your account.</p>
                            <p className={`${secondaryTextColor} mt-12`}>{label} <a href="#" onClick={() => navigate(navigation)} className={`${blueText} font-medium hover:underline ml-1`}>{page} here</a></p>
                        </div>
                        ...
                        <div className="!mt-12">
                            <button onClick={(e: FormEvent) => formSubmit(e)} type="button" className={`${bluebgColor} w-full shadow-xl py-2.5 px-4 text-sm font-semibold rounded cursor-pointer`}>
                                {page}
                            </button>
                        </div>
